...
fn hammer_holder_strict_validation_budget_is_pinned() {
    ...
    // The relaxed path must not run the measured integral at all: its
    // contract skips shell orientation, so a regression that routes strict
    // validation through the relaxed path would read as a speedup while
    // dropping the inside-out check. The strict probes above pin the
    // positive case; this pins the negative one on the same body.
    assert!(
        matches!(
            options.orientation,
            remus_operations::validate::OrientationCheck::Order(_)
        ),
        "strict options must request the orientation integral"
    );
}
