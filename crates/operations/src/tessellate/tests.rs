...
fn sphere_box_cut_equatorial_seam_is_closed_across_deflections() {
    ...
    assert!(
        remus_topology::validation::validate_shell_closed(shell, &topo).is_ok(),
...
fn split_circle_rims_bound_the_torus_snap_range() {
    ...
    fn split_latitude(
        ...
        v: f64,
        parts: usize,
    ) -> (remus_topology::vertex::VertexId, Vec<OrientedEdge>) {
    ...
    let whole_face = remus_topology::explorer::solid_faces(&topo, whole).unwrap()[0];
    assert!(matches!(
