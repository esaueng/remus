...
fn validate_cap_wires_planar(
    topo: &Topology,
    wire_ids: impl IntoIterator<Item = remus_topology::wire::WireId>,
    plane_point: Point3,
    normal: Vec3,
    tolerance: f64,
...
mod tests {
    ...
    fn revolve_partial_curved_boundary_leaving_cap_plane_is_rejected() {
        ...
        let surface = remus_math::surfaces::CylindricalSurface::new(
            Point3::new(0.0, 0.0, 0.0),
            Vec3::new(0.0, 0.0, 1.0),
            1.0,
        )
    ...
    ///
    /// Ported from upstream andymai/remus#1237, which fixed this by
    ...
    fn revolve_segmented_is_outward_for_either_winding() {
        ...
        for ccw in [true, false] {
            for normal_y in [-1.0, 1.0] {
                ...
                let wire =
                    remus_topology::builder::make_polygon_wire(&mut topo, &pts, 1e-7).unwrap();
                let face = topo.add_face(remus_topology::face::Face::new(
                    wire,
                    vec![],
                    FaceSurface::Plane {
    ...
    ///
    /// Ported from upstream andymai/remus#1237.
    #[test]
