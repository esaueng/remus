...
fn assemble_mixed_existing_faces_preserve_exact_arc_trims() {
    ...
    for edge_id in remus_topology::explorer::solid_edges(&topo, source).unwrap() {
    ...
    let specs: Vec<_> = remus_topology::explorer::solid_faces(&topo, source)
        .unwrap()
        .into_iter()
    ...
    let circle_trims: Vec<_> = remus_topology::explorer::solid_edges(&topo, solid)
        .unwrap()
        .into_iter()
        .filter_map(|edge_id| {
    ...
    for (y, z) in [(4.0, -5.0), (8.0, -5.0), (10.0, 0.0)] {
        for swapped in [false, true] {
            ...
            crate::transform::transform_solid(
                &mut topo,
                cylinder,
                &remus_math::mat::Mat4::translation(0.0, y, z),
            )
            .unwrap();
...
mod fallback_policy_refusal {
    ...
    fn corrupted_pair(topo: &mut Topology) -> (remus_topology::SolidId, remus_topology::SolidId) {
        ...
        .unwrap();
        let face = remus_topology::explorer::solid_faces(topo, b).unwrap()[0];
        let v0 = topo.add_vertex(Vertex::new(
            remus_math::vec::Point3::new(50.0, 50.0, 50.0),
            1e-7,
        ));
        let v1 = topo.add_vertex(Vertex::new(
            remus_math::vec::Point3::new(51.0, 50.0, 50.0),
            1e-7,
        ));
