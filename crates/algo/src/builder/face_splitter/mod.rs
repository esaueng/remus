...
fn integrate_holes_plane(
    ...
    for s in &arc_sections {
        ...
        let arc_pcurve = super::pcurve_compute::compute_pcurve_on_surface_in_domain(
            &s.curve_3d,
            s.start,
            s.end,
            s.domain(),
     ...
     -> Result<Option<OrientedPCurveEdge>, AlgoError> {
        ...
        if let Some(&(input_idx, whole)) = sub_edge_src.get(seg_id) {
            ...
            if !whole && inp.is_arc {
                ...
                let domain = match (&inp.edge.curve_3d, trim) {
    ...
    for s in sections {
        ...
        for w in 0..SEC_SAMPLES {
            ...
            for _ in 0..48 {
                ...
                let p = evaluate_edge_at_t(&s.curve_3d, s.start, s.end, s.domain(), m);
            ...
            let pc = evaluate_edge_at_t(&s.curve_3d, s.start, s.end, s.domain(), tc);
        ...
        for w in piece_bounds.windows(2) {
            ...
            let pm = evaluate_edge_at_t(&s.curve_3d, s.start, s.end, s.domain(), tm);
...
fn split_face_2d_impl(
    ...
    // u-band between them. Contained tracer — defers (None) unless the
    // sections form exactly two φ-wrapping loops, each either a chain of open
    // arcs or one closed section. A CLOSED section that does not wrap φ (a
    // small tool poking a disc hole) fails the tracer's winding check and
    // defers to the internal-loops path, which carves it as a hole.
    ...
    for section in sections {
        ...
        let pcurve_on_this_face = if is_plane && !matches!(section.curve_3d, EdgeCurve::Line) {
            owned_pcurve = super::pcurve_compute::compute_pcurve_on_surface_in_domain(
                &section.curve_3d,
                section.start,
                section.end,
                section.domain(),
    ...
    let loop_key = move |wl: &[OrientedPCurveEdge]| -> Vec<(u8, QKey3, (i64, i64, i64))> {
        ...
        let mut keys: Vec<_> = wl
            ...
            .map(|e| {
                ...
                let midpoint = q(evaluate_edge_at_t(
                    &e.curve_3d,
                    e.start_3d,
                    e.end_3d,
                    e.traversal_domain(),
