...
use brepkit_operations::boolean::{BooleanOp, EdgeEvent, EntityEvolution, VertexEvent};
use brepkit_operations::journal_ops;
use brepkit_topology::journal::EntryPayload;
...
impl BrepKernel {
    fn boolean_entity_evolution_json(
        ...
        let (solid, evolution) = brepkit_operations::boolean::boolean_with_entity_evolution(
            self.topo_mut(),
            op,
            a_id,
            b_id,
    ...
    fn linear_pattern_journaled_json(
        ...
        let journaled = journal_ops::linear_pattern_journaled(
            self.topo_mut(),
            solid_id,
            brepkit_math::vec::Vec3::new(direction[0], direction[1], direction[2]),
            spacing,
            count as usize,
        )
        .map_err(StructuredWasmError::from)?;
