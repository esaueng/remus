...
The actual release channel is the committed package: `.github/workflows/publish.yml` ("Refresh Apache Staging Package") runs on every push to `main`, rebuilds `crates/wasm/pkg` with `cargo xtask wasm-build`, and — when the output differs — commits it back as `chore(wasm): refresh committed package to vX.Y.Z from <sha> [skip ci]`. Consumers install by git path (`github:esaueng/remus#main&path:/crates/wasm/pkg`), so that auto-commit IS the release: merging a kernel PR ships it within minutes, no further merge needed. Nothing publishes to npm or crates.io. Cross-repo consumption and the validation harness: see the release-flow skill. Details and the manual refresh dispatch: [reference.md](reference.md), "Committed-package refresh".
