...
Acceptance, per real gridfinity scenario: triangle-count parity, volume parity, manifold/watertight validity, AND wall-clock at least matching the reference kernel. All four, per case. The end goal is replacing the reference kernel's wasm build in `$GRIDFINITY_TOOL`.
...
wasm-pack build crates/wasm --target nodejs --release --out-dir pkg-node-bench
...
2. Point brepjs at it: copy `$BREPJS/vitest.bench.config.ts` to a scratch config and change only the `brepkit-wasm` entry in `resolve.alias` to the absolute path of your `pkg-node-bench/*.cjs`. Do not touch `node_modules`.
3. Run: `cd $BREPJS && BENCH_KERNELS=<ids> npx vitest run --config <your-config> kernel-comparison`. The `BENCH_KERNELS` syntax (single id, comma list) is documented in `$BREPJS/benchmarks/setup.ts`; the actual kernel ids are defined in `$BREPJS/tests/helpers/kernelInit.ts`. Include `brepkit` plus the reference kernel's wasm id.
...
2. Copy the 6 package files into BOTH locations in `$GRIDFINITY_TOOL`: `node_modules/brepkit-wasm/` AND `node_modules/.pnpm/brepkit-wasm@<ver>/node_modules/brepkit-wasm/`. pnpm resolves through `.pnpm`; overlaying only the direct path is the classic wasted-probe mistake.
...
- `./scripts/bench-compare.sh $BREPJS`: unified criterion + JS report into `bench-results/`. It runs `npm install` into brepjs, so it is the slow path; use Procedure 1 for iteration.
