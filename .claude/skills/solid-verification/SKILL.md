...
|---|---|---|---|---|
| 1. Census | Right face count and surface types? | `explorer::solid_entity_counts`, `FaceSurface::type_tag()` over `explorer::solid_faces`; `cargo run --release --example approx_census -p remus-operations` | Tens of faces, quadric types present for curved solids | Geometry is correct. An "exact analytic" result can still have an uncarved feature |
| 2. Validation | Topologically sane? | `remus_operations::validate::validate_solid` (loop-aware Euler) or `remus_check::validate::validate_solid` (report with `CheckId`s) | 0 errors | The shape matches intent. A closed manifold of the WRONG shape passes |
...
| 5. Classification | Is material where intended? | `remus_check::classify::classify_point` (ray-cast) at intent-encoding probe points | Inside carved features: `Outside`. Inside kept material: `Inside` | Anything between probes. A thin missing sliver escapes sparse probes |
...
2. Validate (rungs 2 and 3). Checkpoint: `remus_operations::validate::validate_solid` returns an empty issue list, or only issues you can explain. Free edges must be 0. Note the check-crate `SolidEulerCharacteristic` check demands V-E+F == 2 exactly and will warn on any solid with a through-hole; the operations-crate validator applies the genus and inner-loop correction, so prefer it for boolean results. For assembled compounds that legitimately do not share edges, use `validate_solid_relaxed`.
...
4. Ray-cast spot checks (rung 5). Pick probes AT the feature being verified: pocket centers, wall midpoints, points just inside and just outside each cut face. Use `remus_check::classify::classify_point` with `ClassifyOptions::default()`. NEVER the winding classifier here (see below).
...
- A test-side mesh volume of a small body far from the origin must sum its tetrahedra about a point on the body, as `measure/volume.rs` does for meshes whose directed edges cancel (B56). About the world origin a 1e-3 body moved 13 units reads tenths of a percent off with an unchanged mesh. The origin also hides defects: a hole or inward-wound face in a plane through it contributes nothing, so an origin-centred body can measure right while its mesh is wrong. Check `boundary_edge_count` (half-edge twins), not just incidence. `mass_properties` inertia and centroid are still origin-referenced and drift far away (B58), so compare them in place.
- The check-crate `remus_check::properties::solid_volume` (naive Gauss sum over all faces via `face_integrator::integrate_face`) is NOT a safe exact path for general or boolean solids. It gets trimmed periodic faces and orientation issues wrong; the engine itself only uses it behind narrow guards. Recorded example (PR #959 investigation): a half-disc prism integrated 54.08 against a true 39.27. Do not reach for it as an independent oracle.
...
- Caveat: curved trims are tested as sampled loops (32 points per curved edge), so keep probes clear of them by more than that chord error. A trim loop that wraps a period (a torus band between two pierce loops) is decided by wire orientation, face on the left in the surface's own (u, v), and only on tori: a seamless two-ring cylinder or cone band still reads as empty. Inner shells are covered.
...
2. Census unchanged or improved: `cargo run --release --example approx_census -p remus-operations`. No new approximation probes fire; face counts stay analytic-sized. Re-run this after every boolean-engine change; prior results go stale.
3. Watertight both ways: B-Rep free edges 0 and non-manifold 0; mesh boundary edges 0.
4. Ray-cast spot checks with `remus_check::classify::classify_point` at points that encode the intent of the change.
