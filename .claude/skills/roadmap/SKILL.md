...
- **Equal-radius perpendicular cylinder-union RENDER — NO LONGER TERMINAL
  (2026-08-28, Steinmetz PR).** `exact_cylinder_cylinder` now exists
  (math/analytic_intersection.rs): the equal-radius intersecting-axes pair
  factors into two bisector-plane ellipses, phase FF pre-splits them at the
  pinch crossings (quarter arcs), and the union ships exact at 8 faces with
  the TRUE seam through the pinch vertices, tessellating watertight+manifold
  (pinned by `steinmetz_intersect.rs::perpendicular_equal_radius_fuse_and_cut_stay_exact`).
  The face-split-at-pinch primitive this row asked for is the arc-only region
  promotion in `face_splitter/mod.rs`; the old 6-face marched-NURBS dodge is
  superseded.
...
| **Steinmetz intersect — CLOSED (2026-08-28): equal-radius cylinder ∩ cylinder with intersecting axes is exact analytic.** The quartic factors into two bisector-plane ellipses (`exact_cylinder_cylinder` in `crates/math/src/analytic_intersection.rs`); phase FF emits them pre-split at the two pinch crossings plus half-arc midpoints (the co-endpoint rule), and the periodic-wall splitter promotes arc-only loops strictly outside every seam-carrying band under every periodic translate to REGIONS (`face_splitter/mod.rs` — the structural "arc-only = hole" rule was the blocker). Census row now 6 exact cylinder faces, volume 16/3·r³ to discretization accuracy; ∪/− moved 6→8 and 5→7 faces, still exact, volumes pinned. Holds at any axis angle (16r³/(3·sinθ)) and under rigid motion. Fixtures: `crates/operations/tests/steinmetz_intersect.rs` (5 tests) + math unit tests. THREE tessellation defects the new face shapes exposed, all fixed in the same PR: conic-bounded CDT faces get dense interior rows over the conic v-band (boundary-clearance-filtered — points near constraints fail the CDT into the crack-prone snap path, the lite-pad foil caught it) plus face-own boundary anchors on conic-interior segments (identical shared-vertex ear triangles from BOTH faces of a lens tip read non-manifold); the seam-run CDT repair no longer stretches a partial-height seam to the face v-range nor snaps a correctly-unwrapped seam onto the last rim column (cross_drilled_stl); and per-face `cylinder_has_non_standard_boundary` routes conic-bounded walls off the rectangle grid mesher. | algo/GFA + math + operations/tessellate | Remaining nearby: unequal-radius and skew-axis pairs stay on the marched path by design (irreducible quartic). |
