...
- O2.1b revolution/extrusion math substrate — CLOSED 2026-09-01 in PR #TBD: self-contained swept profiles and both surface carriers provide checked projection, exact derivatives/curvature, periods, and exact directed finite-span rational NURBS lowering across all six profile variants. Exact rational conic lowering preserves point sets and orientation but not angle-linear parameter speed, so differential twin tests compare tangent direction and curvature after projection instead of raw same-parameter derivative magnitude. Revolution projection at an axis pole uses a deterministic one-sided radius derivative; a zero derivative can falsely declare convergence for an off-axis target.
