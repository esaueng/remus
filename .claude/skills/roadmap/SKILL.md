...
B63 SSI branch-point witness (`math/src/nurbs/intersection/tests.rs`), and the
two B64 plane–cone rim fillet witnesses (`blend/src/fillet_builder.rs`
...
- **A closed-rim fillet that passes validity can be the mirror-image blend;** check that the ball centre sits inside the material and that the removed volume matches Pappus — the plane–cone arm shipped a flared-foot torus behind validity-only tests (B64, `blend/src/fillet_builder.rs::closed_rim_oracles`).
