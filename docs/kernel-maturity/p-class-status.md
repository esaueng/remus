...
| 2.0f STEP per-use deterministic round-trip | In review — STEP import binds positioned pcurves to exact coedge uses, preserves analytic surface frames and periodic winding, and fails atomically on count/endpoint mismatch; export is deterministic and refuses inconsistent per-use authority | This PR |
