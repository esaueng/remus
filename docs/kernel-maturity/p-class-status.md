...
| 2.0g Integration, zero gate, corpus, and docs | In review — whole-topology ownership/seam diagnostics run across exact boolean, arena-v3 rollback, external 48-pcurve STEP, and WASM paths; the 132-reader and 30-mutation gates remain zero, unsafe wire mutators are deprecated, and the read-only compatibility facade is retained behind its measured public-API deletion gate | — |
