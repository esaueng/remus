...
| <a id="b32"></a>B32 | **Translation-variant exact cylinder–cylinder cut volume (B26 finding 1)** — the exact cut succeeds, is ops-valid and mesh-watertight, yet measures 4.79 → 13.36 under a rigid translation (the doubled-boundary signature); the tool's side wall is dropped from the result while the caps keep the tool's full disc area. Found 2026-09-16 by the B26 proptest layer (seed `65c59077`). | `operations/tests/prop_boolean_invariants.rs::b26_finding_cylinder_cut_translation_variant`, `operations/src/boolean/`, `operations/src/measure/volume.rs` | S–M | Translation invariance is the doubled-boundary detector; a valid exact result must re-measure identically. Exit: the ignored repro greens with no new mesh fallback. | Open |
| <a id="b33"></a>B33 | **Exact-fuse face-orientation and wire self-intersection supplement failures (B26 finding 2)** — exact fuses succeed with the ops-validator clean yet the check-crate supplement fails: box–cylinder carries one `ShellOrientationConsistent` error at every scale (seed `721153df`; the cyl(1.5,1)×box quarter-turn family fails the ops gate too, so assembly orientation itself is suspect); cylinder–sphere carries `WireSelfIntersection` plus mm-scale `VertexOnCurve` warnings (seed `fd003939`). Found 2026-09-16 by the B26 proptest layer. | `operations/tests/prop_boolean_invariants.rs::b26_finding_box_cylinder_fuse_face_orientation`, `algo/builder/`, `check/src/validate/` | S–M | The supplement is the independent topology oracle; an exact result must satisfy both validators. Exit: the ignored repro greens. | Open |
