...
| B26 | **Boolean and mesh invariant proptests** — the workspace has 15 proptest blocks (13 in `math`) and 8 golden files against ~445k lines, while roughly half of `scripts/` tests the CI policy itself. Add property tests over random primitive pairs and rigid transforms: inclusion–exclusion volume identity (A ∪ B + A ∩ B = A + B), fuse/cut complement, translation invariance of `solid_volume` (the doubled-boundary detector), watertight/manifold mesh, exact-only path stability under 1e-13 nudges. | `operations/tests/prop_boolean_invariants.rs` (new), `operations/src/boolean/tests.rs` | S–M | Cheapest broad oracle the kernel lacks; every lesson in the roadmap skill about doubled boundaries and translation-variant volume is a property nobody generates. Rides B19's schedule for the slow variants. | **Partial (2026-09-12):** `prop_boolean_invariants.rs` landed with 7 generated families (box-pair, box-cylinder plug, cavity, rigid, scale, spheres, nudge) over hand closed forms + set identities + dual validators + position-quantized edge recount + watertight mesh, with exact/refusal/incorrect taxonomy, non-vacuity gates, shrinking + deterministic seed replay, and a documented opt-in campaign (`PROP_BOOL_CASES`/`PROP_BOOL_SEED`). CI replay is 26 exact_ok / 0 incorrect in ~5 s; opt-in seeds 11/12/13×40, 21×200, 22×64 green; nightly `boolean_tree` fuzz 120 s / 2860 runs clean. Zero kernel defects found — every red minimized to a harness-oracle bug fixed in the harness. Remaining: curved fuse/cut in CI scope (sphere cut ~36–66 s/call keeps it opt-in only), cone/torus boolean families, second-model generality. |
