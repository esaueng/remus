...
| B24 | **Wildcard-arm audit and gate** — 182 `_ =>` arms in 57 production files over `EdgeCurve`/`FaceSurface` (gate `scripts/check-wildcard-arms.sh` + `scripts/wildcard-arms-baseline.txt`, same pattern as `check-det-hash.sh`; densest: `measure/volume.rs` 16, `pave_filler/phase_ff.rs` 16, `tessellate/nonplanar.rs` 13 — incl. the stepped-rim `level_of` arm from #403 — `resize_blend.rs` 10). A new variant compiles clean and silently takes the approximate branch in volume, meshing, and GFA splitting. Convert the four densest files to exhaustive matches. Companion to RFC 0006 (face-surface wildcard audit). | `operations/src/measure/volume.rs`, `algo/src/pave_filler/phase_ff.rs`, `operations/src/tessellate/nonplanar.rs`, `operations/src/resize_blend.rs`, `scripts/` | M | The only current defence is `approx_census`, which detects drift after the fact. RFC 0006 swept-analytic surfaces will add a variant and hit every one of these. | Partial — ratchet script + baseline land here rebased on current `origin/main` and still unwired (fleet-ci.yml is immutable-pinned: wiring it in would fail the caller-pin tests); the `repo-policy` one-line wiring lands with the pin update, then the four file conversions. |
