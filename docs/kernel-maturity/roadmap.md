...
| B12 | **Holes on non-planar section caps** — annular Coons or cap-then-subtract vs extruded-annulus ground truth (stabilization B2.2). | `operations/src/cap.rs`, `fill_face.rs` | M | Largest remaining non-planar-cap value with clean ground truth. H3, with M7 cap work. | **Done (2026-09-04):** sweep/pipe preserve certified rectangular iso-parametric holes on 4-sided bilinear caps; extruded-annulus volume, topology, mesh, classifier, direct-WASM, and batch witnesses pass. Broader trims and loft-hole correspondence refuse typed. |
