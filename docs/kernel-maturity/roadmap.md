...
Snapshot from GitHub on 2026-09-17. Re-fetch before claiming or changing any row.
The 2026-09-13 inventory (#410, #413, #416, #417, #422) is fully closed or merged;
its review findings stay in the table below until each owner row integrates them.
No PR check result or reviewer approval is inferred from this inventory.

| PR | Scope / owner | Inspected head |
| --- | --- | --- |
| [#493](https://github.com/esaueng/remus/pull/493) | Rigid-transform certificate budget slack (#483); strict-arena wasm refusal | `23a77cec` |
...
| <a id="b47"></a>B47 | **Box–cylinder fuse open mesh at fine deflection (B26 finding 22)** — the exact fuse of box 2×1×1 with cylinder r=3/h=3.5 (z-rotated 0°, offset (4.25,-1.25,-1.75), unit scale) is fully valid (strict validator clean, 9 faces) with sane volumes (inclusion–exclusion holds: fuse 100.766 + inter 0.194 = box 2.0 + cyl 98.960; inter/cut legs watertight at every deflection), yet the fuse leg tessellates open (54 boundary edges, 0 non-manifold) at the scale-derived deflection (~1e-4) while closing at coarse deflection (watertight at 0.1, 6 boundary at 1e-2) — a tessellation-density crack on the fuse seam, same mesh-class signature as finding 19 (B43) on different shapes and placement, so a distinct finding and row until a shared root is proven. Found 2026-09-17 by the B26 proptest fast suite during the PR #491 coverage run (no persisted seed — shrinking aborted, the repro is the retention). Root cause (2026-09-17): the hole-aware cylinder mesher (`tessellate_revolved_with_holes`) seeds every angular column at the midpoint between the outer rims; this box straddles the wall's mid-height, so the seeds sat exactly on the pocket's bottom-arc constraint, the CDT recovered the constraint through them, and the rim gained vertices the shared pool never sampled (the `#696` class through a different door). Interior seeds and grid points on any wire constraint are now dropped; watertight at 0.1/1e-2/1e-3/1e-4. The same seeds also owned B43 (finding 19, both repros) and B34 (finding 8): all three flip from failing to passing with only this function changed. | `operations/tests/prop_boolean_invariants.rs::b26_finding22_boxcyl_fuse_mesh`, `operations/src/tessellate/` | S | A valid exact B-Rep must tessellate watertight at every deflection. Exit: the ignored repro greens. | Done 2026-09-17 (PR #507) — `b26_finding22_boxcyl_fuse_mesh` un-ignored and green at 0.1/1e-2/1e-3/1e-4; fix in `tessellate/planar.rs` (`tessellate_revolved_with_holes`) |
