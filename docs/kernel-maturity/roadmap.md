...
| <a id="b10"></a>B10 | **Curve-curve / curve-surface classification qualification** + conic distance/classification cells | `math`, `geometry/extrema`, matrix harness | M | Unqualified since the matrix was written; sits under many families' claims; pure evidence. | **Partial — first evidence matrices landed:** `geometry/tests/b10_curve_curve.rs` (twin line/circle × disjoint/tangent/crossing/coincident/near-tangent-10·tol × 1e-3/1/1e3; 7 green, 2 ignored transversal-crossing seeds), `geometry/tests/b10_curve_surface.rs` (circle/line twins × exact plane/cylinder twins × configuration × scale × rigid transform; 9 green), `geometry/tests/b10_conic_distance.rs` (line/circle/ellipse extrema vs closed forms + stationarity; 5 green, 3 ignored carrier/parity seeds). All oracles independent (closed forms, implicit scans, both-geometry re-evaluation); near-tangent/tangent tolerance behavior recorded, never widened. Seeds owned here: bezier-clip misses transversal conic-twin crossings (fat-line clip converges to the control-polygon midpoint, `clip_to_fat_line`); `Parabola3D`/`Hyperbola3D` lack `ParametricCurve` (math `traits.rs`); `ellipse_to_nurbs` twin is approximate (semi-axis-index scaling) and `point_to_curve` Gauss-Newton stalls at minor-axis vertices (dropped curvature term equals the kept term). Remainder: analytic CC/CS arms, quartic-seam cells, NURBS×NURBS matrix (shared with 2.4c/2.5), cone/sphere/torus sampled-twin approximate cells. |
...
| <a id="b39"></a>B39 | **Torus–cone pair-cell misbuilds (B26 finding 14)** — at 1e-3 (x-rotated 3π/2, offset (2.5,0,0)) the cut is ops-invalid (inconsistent orientations) while the fuse is fully valid yet tessellates open (314 boundary) with an 11% drift and the intersect refuses; at unit scale (y-rotated 3π/2, offset (1.5,2,1)) the fuse is fully valid yet its mesh opens at fine deflection (watertight at 0.1, 254 boundary at 0.01/0.001 — a tessellation-density crack, distinct from finding 8's open-at-every-deflection caps) and the cut is ops-invalid (2 inconsistent edges), beside a refusing intersect and broken material accounting. Torus–cone pairs excluded from generation until close (no oracle gates — nothing drawn). Found 2026-09-16 by the B26 proptest slow suite. Bisect note (2026-09-19): no commit changed the unit fuse from valid-open to refused — the pinned repro refuses (`ExactOnlyUnattainable`, GFA degenerates to a 1-face open shell) on every commit from 2026-08-31 to `origin/main`; see `b39-bisect-2026-09.md`. Root-cause note (2026-09-20): traced below the gate — the pierce boundary is a *composite* of per-pair FF sections (cone-wall arcs + cap-plane ovals) whose chain junctions never land on the same exact rim∩torus crossing (the analytic marcher ends its arcs one full step short of the rim), so the arrangement band tracer defers and the generic paths misbuild; a marcher-level repair attempt (exact band-boundary bisection + analytic-extent endpoint refinement) made the oblique band emit but broke two WASM qualification cells (`batch_offset_torus_sphere/cone_sphere_boolean_retains_exact_carriers`) and was reverted — see the bisect doc's appendix for the remaining phase-FF path. | `operations/tests/prop_boolean_invariants.rs::b26_finding14_toruscone_cut_invalid`, `::b26_finding14_toruscone_unit`, `::b26_finding14_toruscone_sibling` (same shapes at offset (2.5,-0.5,0): fuse AND cut ops-invalid, intersect refuses — no persisted seed, the repro is the retention), `::b26_finding14_toruscone_oblique_drift` (unit, oblique 45°: valid B-Rep, watertight mesh, ~16% translation drift — finding-1 class on this pair), `algo/builder/`, `operations/src/tessellate/` | S–M | One pair-cell, two scales, four placements, same family. Torus–cone pairs excluded from generation until close. Exit: all four ignored repros green. | **Open — partial (2026-09-17):** the two 1e-3 placements (`b26_finding14_toruscone_cut_invalid`, `b26_finding14_toruscone_sibling`) were closed by PR #490 (B45 — closed tube-wrapping torus sections traced as band loops; bisected: fail at `aa444568`, pass at `281afaa5`; deterministic over three runs on `3785eebc`) and are un-ignored. `b26_finding14_toruscone_unit` (GFA degenerates to a 1-face open shell, typed refusal) and `b26_finding14_toruscone_oblique_drift` (composite-pierce assembly fails, typed refusal on 2026-09-20 — the earlier ~16% drift was masked behind a validity failure) still fail on `b13ff97b`; generation exclusion stays until they green. |
