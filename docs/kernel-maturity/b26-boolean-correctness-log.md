...
1. Run required repo checks (workspace tests affected scope, boundaries,
   doc paths, WASM smoke if touched — not touched).
2. Update roadmap B26 row (Partial: harness landed; campaign green with
   zero kernel findings preserved as follow-up corpus, not repairs).
3. Open ready-for-review PR with the approved Git identity; report link +
   counts. Do NOT merge.
