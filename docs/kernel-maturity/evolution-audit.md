...
| Linear pattern | Total F/E/V construction claims for source and copied instances | `crates/operations/src/journal_ops.rs`: `linear_pattern_journaled`; native result-census/instance-position and WASM direct/batch contracts | Copied outputs are Generated, not identity successors of source references; circular pattern history remains separate |
