...
| ID | Outcome | Reference capability | Disposition | Contract | Competitive | Evidence | Owner |
|---|---|---|---|---|---|---|---|
| IP-10.1 | Mass, area, volume, centroid, inertia | Documented (Gauss integration) | In-scope | Exact planar boundary moments (line/circle/ellipse/parabola/hyperbola/recognized NURBS); deflection-independent analytic curved-face area and exact-quadrature centroid; volume/centroid/inertia with stated bound, scale-matrix proven | Parity-proven on the B20 matrix (filleted/cavity × 1e-3/1/1e3, ≤ 1e-6 vs closed forms, native + direct/batch WASM) | K-S2 disposition; `properties/face_integrator.rs`; OpenZCAD `docs/kernel-roadmap-remus.md` S2 | B20 done |
