...
- B5 offset face provenance — CLOSED 2026-09-02 in PR #TBD: the default intersection-joint V2 offset retains and validates its exact one-to-one source-face map, journals it transactionally as construction evolution, and exposes matching direct/batch `offsetJournaled` WASM contracts. Plane displacement, 3x3x3 closed-form volume, persistent-reference chaining, atomic refusal, and WASM package oracles pin the claim; arc-joint and self-intersection-removal variants fail closed because their richer generated/replaced-face provenance remains future work.
