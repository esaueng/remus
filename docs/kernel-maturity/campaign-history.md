...
- B6 analytic primitive matrix — CLOSED 2026-09-03: box, cylinder, pointed cone, frustum, sphere, and torus pass native/direct/batch scale and complete-postcondition oracles in `qualify_primitives.rs` and WASM `qualify_primitives_tests.rs`; the invalid matrix found and closed non-finite box/sphere/torus acceptance. The same oracle rejected the WASM-only ellipsoid as a separate blocked cell: its transformed-sphere NURBS tessellation is empty at unit/large scale and leaky with about half volume at 1e-3, pinned by `ellipsoid-tessellation-scale.json` and the ignored acceptance target.
