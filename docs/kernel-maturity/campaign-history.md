# Campaign history — closed roots, refuted theories, and the digs behind them

Moved verbatim out of `.claude/skills/roadmap/SKILL.md` on 2026-09-02 so the
skill carries only doctrine. Nothing loads this file by default; read it
when a task resembles something a past session already tried, when a closed
row's *reasoning* matters, or when the roadmap skill's durable-lessons list
points here. The queue is `roadmap.md`; the doctrine (chase filters,
TERMINAL list, acceptance bar) is the skill. Every entry here is history:
...
## Conic-distance seeds: a dropped curvature term and two wrong oracles (B10, 2026-09-25)

The three ignored B10 conic-distance seeds turned out to be one solver defect plus two wrong premises. Adding the missing `ParametricCurve` impls for the parabola and hyperbola turned their `panic!` placeholders into real failures, and every one traced to the same place: `point_to_curve` and `curve_to_curve` stepped with the Gauss-Newton derivative `|C'|²`, dropping `(C − P)·C''`, which is not small at a positive distance. At the ellipse's minor-axis vertex seen from 4 radii the dropped term equals the kept one, so each step was exactly twice too long and the iterate orbited; at a hyperbola vertex it is twice the kept term, so the step was three times too long and diverged (20% distance error at scale 1e-3); and for a line against a parabola's vertex tangent the Gauss-Newton matrix is exactly singular (2x distance error). The fix takes the full Newton step with exact carrier derivatives (a new defaulted `derivative_pair` on the trait), falls back to Gauss-Newton or a scaled gradient when the Hessian is not positive definite, and backtracks. A second layer showed up at the 1e-9·scale parity band: backtracking on distance alone stalls at ~sqrt(eps) relative, because the squared distance is flat to second order at its minimum; steps are now also accepted when the stationarity residual halves. A first attempt with finite-difference derivatives left a 6e-9 miss on the NURBS twin because the foot sat on a double knot where the stencil straddles a jump in `C''`, which is why exact derivatives went into the trait.

The seeds' own oracles were wrong in two ways. The parabola query (0, 4f) and the hyperbola query (5, 0) for (2 cosh t, sinh t) both lie beyond the vertex's centre of curvature, where the vertex is a local distance MAXIMUM; the closed forms put the minima off-axis (distance 2√3·f and 2). And the twin-parity seed measured the ellipse's implicit residual in a fixed x-major frame while `Ellipse3D::new` puts the major axis on +Y for a +Z normal (the same trap that made the curve-curve ellipse seed's ellipses disjoint), so it read ~3 on a twin exact to 1e-16. Cells: `crates/geometry/tests/b10_conic_distance.rs`; derivative check: `remus_math::traits` tests.

## Bezier-clip fat line re-used the parent polygon at every depth (B10, 2026-09-25)

The two ignored B10 curve-curve seeds (transversal circle-twin and ellipse-twin crossings returning zero hits) were traced by printing each depth's clip windows on the minimized quarter-arc seed. `clip_to_fat_line` received the parent segment's control points at every depth and only the window's t-coordinates narrowed, so every clip mapped the same distance polygon onto a smaller window: the result was a fixed centred shrink to [1/4, 3/4] per level on both curves. The windows walked to parameter 0.5, the true root (A-u 0.5378) left A's window [0.46875, 0.53125] at depth 7, and the sampled-AABB prefilter pruned the pair at depth 8. The same defect made the math bench's own cubic pair return zero of its three crossings.

The fix blossoms both windows out of their parent segments at every depth. It also replaces the non-affine `|v × dir|` signed distance with the affine `(P − a0)·m`, and clips rational windows through their weighted distance numerators. Sound clips exposed two things the old midpoint-walk had hidden. Coincident windows no longer stay aligned (a prefix window never passes a Hausdorff test), so the overlap check now trims both windows to the stretch whose end-points lie on the other curve. Tangent contacts are within tolerance over a well that grows as √(tol/κ_rel) with scale, so the Hausdorff test called them overlaps at scale 1e3 and at tol 1e-9, and tiling that well down to tolerance-size windows cost ~66 s per contact in debug. Overlaps now also need second-order (tangent + curvature) agreement across the segment size, and a window lying wholly within tolerance of the other curve is refined once as a single contact. Termination, Newton (polish to scale-relative float resolution, accept at the tolerance), and duplicate merging (3D point or tolerance-connected stretch) moved to model space. A latent role-swap bug that reported `u1` on the second curve at odd swap parity was fixed along the way. The ellipse seed itself was wrong: `Ellipse3D::new` picks +Y as the major axis for a +Z normal, so its two ellipses were disjoint and the seed failed in its own oracle, never reaching the clipper. Evidence and cells: `crates/geometry/tests/b10_curve_curve.rs`, `crates/math/src/nurbs/bezier_clip.rs` tests, roadmap row B10.

## Fleet-fuzz dangling pin breaks every PR's Classify Changes job (2026-09-18)

Every PR's Classify Changes job started failing with `fatal: path '.github/workflows/fleet-fuzz.yml' exists on disk, but not in 'de062d7c06c85ee5cf524aca019d8a73485e61e8'`, first seen on #493's re-run at `23a77cec` on 2026-09-18 01:41 UTC while the same branch's earlier run at `291d2d16` passed. The caller pin in `.github/workflows/fuzz.yml` points at `de062d7c`, a pre-squash branch commit of #441 whose source branch has since been deleted, so the `git show <sha>:<path>` resolution used by `scripts/test-ubuntu-ci-routing.py` (Ubuntu scheduling, immutable guards, and migrated job contracts) and `scripts/test-direct-fleet.py` (checked-in scheduling expressions against trusted and hostile events) fails; every other fleet pin is a `main` ancestor and only `fleet-fuzz` dangles. The `docs/kernel-maturity/roadmap.md` proof-gate note still records that pin as reachable.

The fix is proposed, not landed: [PR #499](https://github.com/esaueng/remus/pull/499) (OPEN, created 2026-09-18T01:53:02Z) repoints `fuzz.yml` to `055b7fc0`, the #441 squash commit on `main`, whose `fleet-fuzz.yml` content is byte-identical to the dangling pin's, leaving trusted workflow content unchanged. Runner proof is pending — the PR's own Classify Changes job is the proof.

## Sequential fillet across an earlier band (2026-09-17)

Filleting a box edge after the neighbouring edge was already filleted was refused: the seed stopped at the tangent seam where the wall turns into the earlier fillet's band, and no engine could end a blend there (the OpenZCAD "Fillet could not be created on 1 selected edge" case, and the #834 blend-adjacent test that was pinned as a typed refusal). The fillet now follows its tangent ridgeline across the band's arc onto the far edge, and the corner becomes a sphere (equal radii) or a torus band (smaller radius). See [PR #496](https://github.com/esaueng/remus/pull/496).

The roots were a G1 chain expansion that stopped at any seam, a rolling-ball engine with no junction there, a singular equal-radius corner, and a curved-edge wall lofted through the sharp-edge apex. Chain expansion in `crates/blend/src/g1_chain.rs` now crosses a tangent seam when the outward normals agree pairwise and every changing face changes surface; a coplanar split of one plane stays a boundary, so the OpenZCAD bracket still fillets its base-plate corners only. `crates/operations/src/fillet/rolling_ball.rs` treats the junction as a chain vertex and snaps shared contacts by nearest point, and `crates/blend/src/fillet_builder.rs` emits the degenerate equal-radius corner in closed form (`equal_radius_cap`); full-turn rims stay refused by the curvature guard. The wall now lofts exact rational arc sections via `remus_math::nurbs::interpolate_with_params` instead of a parabola through the apex (corner cap area 2.61 vs the octant's 1.57 before; tracks the sphere to 4e-5 after). The walking builder refuses a seam-crossing chain by name (`unsupported-seam-crossing`, `BlendError::UnsupportedSeamCrossing`) so the cascade hands it to the rolling-ball engine.

The repro pair `sequential_fillet_equal_radius_rounds_corner_as_sphere` / `sequential_fillet_smaller_radius_rounds_corner_as_torus` in `crates/operations/src/fillet/tests.rs` runs the app scenario (9.5 x 12.5 x 8.5 box, vertical edge r=1, then the top edge) through `fillet_cascade` with full validity, entity counts, closed-form volume, and ball-centre distance; `g1_chain_does_not_cross_a_coplanar_split` and `equal_radius_full_rim_is_still_refused` guard the two stays-refused boundaries, and `fillet_edge_adjacent_to_blend_is_watertight` (#834) now asserts success and the convex volume drop. The torture corpus carries `fillet-the-fillet` (typed refusal) and `fillet-the-fillet-cascade` (Built) in `crates/operations/tests/fillet_torture.rs`, mirrored by the WASM contract test `fillet_top_edge_after_vertical_edge_fillet` in `crates/wasm/src/bindings/sequential_fillet_tests.rs`. Behaviour change: a seed whose ridgeline continues across a genuine tangent seam now rounds the whole ridgeline. The Gridfinity D5 lip is still a typed refusal, now named as the seam crossing.

## Sphere frames dropped by rigid moves (2026-09-17)

The B26 finding-16 witnesses behind row B41 — a disjoint cone–sphere fuse at 1e3 (frustum r0=1000/r1=500/h=1000, sphere r=1000 at (0,-1000,3000)) and its unit twin (sphere r=1 at (0,-2,3)) — were fully valid 5-face B-Reps whose exact route measured the 6.02e9 / 6.02 closed-form sums while the mesh route tessellated open (16 and 8 boundary edges) and read 8% and 6.6% low, the unit case additionally drifting 60% under rigid translation. The Fuzz Smoke box–sphere case (box 1×1.5×1.5 with a disjoint r=1.5 sphere rotated 45° about Y at (−2,2.5,2.5)) showed the same signature: valid, `mass_properties` exact at 16.3872, yet 13 open seam edges and `solid_volume` 12.7762 (−22%). None of it was the boolean; the row's suspected single tessellation-drop root turned out to be three roots, all in the sphere primitive's rigid-motion path.

`transform_solid`/`transform_face` rebuilt every sphere with `SphericalSurface::new(center, r)` on world axes, so a rotated primitive's hemispheres kept `z` up while their shared equator polygon tilted and the rectangular sweep over-covered the rim (13 extra flap triangles on one hemisphere, one per rim segment). `compute_sphere_v_range` picked each hemisphere's pole from the rim's signed area projected onto the world XY plane — a degenerate line for X/Y-rotated frames whose area is rounding noise, opposite per face near the origin but identical far from it, so both hemispheres swept the same half (per-face triangles `[10011, 0]`, 141 open rim edges). `tessellate_nonplanar_cdt` rescaled a noise-height boundary (parametric height ~1e-13 on a far-placed constant-v rim) up to a square box and fanned slivers over it.

The frame is now carried with `with_frame`, as cones and tori already were (`crates/operations/src/transform.rs`); the pole choice winds the rim about the sphere's own axis (`crates/operations/src/tessellate/nurbs.rs`); the CDT path declines any boundary of negligible height or width (1e-9 relative) so the sweep takes the face exactly as at the origin (`crates/operations/src/tessellate/nonplanar.rs`). The dead world-XY `projected_signed_area` helper is removed. `crates/operations/tests/regress_sphere_transform_frame.rs` pins the carried frame, opposite poles at five rotated/far placements, and closed-form volumes through both routes with watertightness and translation invariance; the two `b26_finding16_*` witnesses in `crates/operations/tests/prop_boolean_invariants.rs` are un-ignored and green, and the fuzz unit is committed as `fuzz/corpus/modifier_ops/box-fused-with-rotated-disjoint-sphere`. Closed by [PR #495](https://github.com/esaueng/remus/pull/495) (2026-09-17). Deliberately untouched: `shell_op.rs`, `offset/src/offset.rs`, and `offset_face.rs` rebuild spheres on a radius change, not a motion — named on the row rather than changed blind.

## Valid-B-Rep open-mesh family: B47 pinned beside B43 and B34 (2026-09-17)

[PR #494](https://github.com/esaueng/remus/pull/494) (merged 2026-09-17) pins B26 finding 22, drawn by the PR #491 coverage run of `prop_random_primitive_pair_identities`, as roadmap row B47. The exact fuse of box 2x1x1 with cylinder r=3/h=3.5 at (4.25,-1.25,-1.75) is fully B-Rep-valid with sane volumes, yet tessellates open with 54 boundary edges. The change is tests + docs only, kernel untouched: one `#[ignore]` ready-repro, an exact-input gate skipping only the fuse-mesh oracle, and the B47 row plus B26 enumeration update.

The shared signature is a fully valid exact B-Rep that tessellates open. B47 (`b26_finding22_boxcyl_fuse_mesh`) carries it with 54 boundary edges; B43 (`b26_finding19_grazing_fuse_mesh` with 6, `b26_finding19_grazing_sibling` with 42) on the 0.05-unit grazing sliver; B34 (`b26_finding_cylinder_cap_closed_circle_mesh_open`) with 210. All four repros live in `crates/operations/tests/prop_boolean_invariants.rs`.

The discriminants keep them distinct. B34 is root-caused to the tessellator, not the boolean: the stock cap survives as a single closed-circle face of unmerged split-rim arcs (`crates/heal/src/upgrade/merge_split_rim_arcs.rs`), open at every deflection 0.1 down to 1e-5 whenever the tool bites the cap off-center. B47 closes coarse (watertight at 0.1, 6 boundary at 1e-2) and opens only at the fine scale-derived deflection (~1e-4), a tessellation-density crack, with inter/cut legs watertight throughout. B43 opens on the sliver seam with exact volumes and translation to 1e-15. B47, B43, and B34 stay OPEN as distinct rows until a shared root is proven.

## Closed tube-wrapping torus sections filed as holes (2026-09-17)

Fuzz Smoke run 35180928629 failed `modifier_ops` with "fillet: tessellation ... not watertight — 254 boundary edges". Replayed natively, the fillet was innocent: the fused base body — a torus (R 4, r 0.5) pierced by a cone frustum whose base cap is a plane through the torus axis and whose wall the tube leaves obliquely — already meshed open at the harness deflection. Both volume routes misread it inside the harness's agreement band (`solid_volume` 743.58, `mass_properties` 742.66, true 738.30), so only the watertight check fired, on the fillet that followed.

Both pierces are single closed tube-wrapping sections, but `split_torus_band_by_arrangement` accepted only stitched open arcs (the box family), so both loops fell to the internal-loops path as inner wires of the untouched periodic outer wire. That form passes the free-edge census, Euler, and `validate_solid`, and the torus mesher skins the "holes" over with zero rim edges — the same "two tube-wrapping rims bound a band" lesson as the 2.4b notch, extended from box-wall open arcs to closed pierce sections.

In `crates/algo/src/builder/face_splitter/special_cases.rs` a closed section now enters the band tracer as a one-edge loop; the existing φ-winding check still defers a closed section that does not wrap the tube (a disc hole) to the internal-loops path, junction snapping skips one-edge loops, and each loop's ring angle is averaged over samples along its edges. The `crates/algo/src/builder/face_splitter/mod.rs` gate no longer requires all-open sections. The output is the 2.4b band form (outer wire one rim loop, inner wire the other, opposite tube traversal): watertight at 0.1 / 0.01 / harness deflection with both integrators reading 738.30 within 0.2 %, pinned by `crates/operations/tests/regress_torus_pierce_band.rs` and corpus seed `fuzz/corpus/modifier_ops/fillet-after-torus-pierced-through-frustum-cap-and-wall` ([PR #490](https://github.com/esaueng/remus/pull/490)). The fillet band's own coarse-deflection opens stay open as B46, and B39/B44 (torus pairs whose valid fuses mesh open) may share this root — re-run their witnesses on this head before chasing them separately.
...
## Ledger closures formerly listed in the skill (2026-08-30 → 2026-09-01)
...
- O1.1d gauntlet triage loop — PARTIAL 2026-09-01 (1/5 classes, PR #177): generic period-winding STEP bands reconstruct exact analytic seams or refuse transactionally; `crates/io/tests/step_generic_face_bound_order.rs` pins MAMBO B12, and smoke moved `invalid_input` 14→10 / full passes 26→29.
...
- O1.4a STEP validation properties — CLOSED 2026-09-01 in PR #180: opt-in CAx-IF aggregate and per-solid validation properties round-trip with derived units, stable typed diagnostics, transactional refusal, and direct/batch WASM parity.
- O2.1a RFC 0006 swept analytic surfaces — CLOSED 2026-09-01 in PR #183: accepted design fixes the self-contained swept-profile representation, STEP-compatible parameter/seam rules, checked projection and typed refusal contracts, exact lowering/recognition, R8 staging, and a measured 92-site production `FaceSurface` wildcard audit.
- O2.1b revolution/extrusion math substrate — CLOSED 2026-09-01 in PR #189: self-contained `SweptCurve` profiles and the two surface carriers provide checked projection, exact derivatives and curvature, explicit periods, and exact directed finite-span rational NURBS lowering for all six profile variants without changing topology. Differential oracles cover scale, seam, pole, reversed spans, success, budget exhaustion, cancellation, and malformed inputs. Exact rational conic lowering preserves the point set and orientation but not angle-linear parameter speed, so twin tests compare positions after projection plus tangent direction and curvature; revolution projection uses a deterministic one-sided radial derivative at an axis pole to avoid falsely declaring an off-axis target stationary.
...
- P2.8 SSI marcher-budget WASM surface — CLOSED 2026-09-02 in [PR #202](https://github.com/esaueng/remus/pull/202): direct quality/cancellation and batch quality booleans expose the existing march/queue/segment/branch caps with default, boundary, rejection, generated-WASM, and rollback contracts; parameter-space tolerance and wider operation adoption remain in `docs/kernel-maturity/p-class-status.md`.
- P3.3 entity-tolerance predicate plumbing — CLOSED 2026-09-02 in PR #208: declared edge tubes and vertex balls widen EE crossing/AABB, forced-overlap, pave-vertex lookup, VE incidence, and SameParameter/SameRange validation only by their excess above the global floor; no-declaration foils preserve existing behavior, invalid bands refuse typed, and the 51-row approximation census is unchanged. Result-tolerance growth plus FF/builder assembly remain P3.4.
- P4.1 RFC 0005 body taxonomy — CLOSED 2026-08-30 in PR #127: the staged design maps the capability matrix's solid/sheet/wire/general-body axis to validation, side-of classification, Compound-first cellular results, STEP entities, evolution, and WASM contracts. Implementation remains P4.2–4.7.
- P4.2 body-class validation substrate — PARTIAL 2026-09-02 in PR #209: RFC 0005 Stage 1 adds public body-class tags, class-aware solid/sheet/wire validation with stable diagnostics, and backward-compatible optional arena-v3 fields. Standalone sheet roots and L3/STEP/WASM entry points remain for later tranches; the substrate alone promotes no capability cell.
- P4.2 sheet operations and WASM — PARTIAL 2026-09-02 in PR #210: transactional face-set construction validates before commit; sheet area and typed volume refusal dispatch by body class; open tessellation preserves a trimmed NURBS patch and a sub-deflection triangular hole; direct and batch WASM contracts agree. Standalone arena-v4 roots, bounding box, center-of-area, and STEP round-trip remain, so the Issue 4.2 exit gate and capability promotions stay open.
- P4.2 standalone sheet arena roots — PARTIAL 2026-09-02 in PR #211: additive arena v4 preserves standalone sheet root order and duplicates plus exact trimmed-NURBS/coedge-pcurve authority; legacy v1-v3 readers remain, v3 writer bytes are frozen, solid-only readers reject sheet roots before replay, and native/WASM malformed-class and resource-limit refusals are transactional. Bounding box, center-of-area, and STEP round-trip remain, so the Issue 4.2 exit gate and capability promotions stay open.
- P4.2 sheet spatial properties — PARTIAL 2026-09-02 in PR #212: body-class-checked sheet bounding boxes reuse conservative trimmed face-set bounds; center-of-area integrates exact face geometry with bounded quadrature and subtracts offset holes; direct and batch WASM agree and solid-class shells refuse with the stable typed measurement diagnostic. STEP round-trip remains, so the Issue 4.2 exit gate and capability promotions stay open.
- P4.2 STEP sheet exchange — IMPLEMENTED 2026-09-02 in PR #213: body-aware native APIs and direct/batch WASM map tagged sheet roots through `SHELL_BASED_SURFACE_MODEL` over `OPEN_SHELL` or `CLOSED_SHELL`, retain representation-scoped tolerance authority, keep solid and sheet handles distinct, and leave legacy solid-only entry points unchanged. A trimmed bilinear NURBS patch passes deterministic write → read → write plus validation/measurement checks; malformed constituents and wrong body classes refuse transactionally. This completes the Issue 4.2 implementation exit witness.
- P4.3 split solid by sheet — IMPLEMENTED 2026-09-02 in PR #214: the GFA arrangement admits a first-class sheet as a face-set tool without classifying it as a volume or selecting it as an ordinary boolean result. The bounded single-cylinder-face witness splits a box into an inside cylinder cell and outside remainder in a Compound; both validate, their closed-form volumes sum to the box, repeated native runs and direct/batch WASM agree, and unsupported sheet shapes preserve the stable `unsupported_sheet_split` diagnostic transactionally. Multi-face, NURBS, and other analytic sheet tools remain unqualified.
- P4.4a trim sheet by solid — PARTIAL 2026-09-02 in PR #215: keep-inside and keep-outside select only classified rank-B sheet patches from the face-set GFA arrangement, export a new validated Sheet, and preserve deterministic native/direct/batch WASM behavior. A planar sheet crossing a box returns exact 100-area and 96-area remainders; empty selections and both aliased and separately built coincident patches refuse transactionally with `unsupported_sheet_trim`. Mutual sheet×sheet trim and the trim-plus-sew exit witness remain P4.4b.
- P4.4b planar sheet×sheet trim and sew — IMPLEMENTED 2026-09-02 in PR #216: effective face normals define positive/negative side selection for composable one-way and strict two-result mutual trims; all results are new validated Sheets with direct/batch WASM parity and stable typed refusals. Six outward-oriented planar carrier sheets trim boundary by boundary, sew into a valid six-face solid, match the exact `make_box` volume, and repeat deterministically. This closes the Issue 4.4 implementation exit witness; curved, multi-face, coincident, and non-dividing sheet pairs remain unqualified.
- P4.5 planar solid imprint — IMPLEMENTED 2026-09-02 in PR #217: the GFA split arrangement keeps every target patch in a new validated solid without material removal, while construction records journal split faces as repeated `Modified`, section edges as two-face `Generated`, and the untouched tool as `Preserved`, with no `Deleted` or `Unresolved` events. The bounded box witness preserves exact volume, resolves a pre-imprint face reference `BoundMany`, repeats deterministically, and matches direct/batch WASM; aliased, non-dividing, same-domain, curved, and incomplete-lineage cells refuse with `unsupported_imprint` transactionally.
- P4.6a two-solid cellular booleans — PARTIAL 2026-09-02 in PR #218: BuilderSolid returns disconnected growth shells as deterministic independent solids and assigns cavity shells to the smallest containing region, while the existing single-solid ABI is an explicit compatibility fold. Exact `boolean_regions` returns a Compound with independently validated members and total non-unresolved construction lineage. A through-slab cut severs a 10-cube into two exact 400-volume regions; disjoint boxes fuse as exact 10/24-volume regions; native and direct/batch WASM results repeat. Compound operands, recursive provenance composition, and deletion of the legacy multi-region-solid helpers remain P4.6b.
- P4.6b Compound-operand cellular booleans — IMPLEMENTED 2026-09-02 in PR #219: exact transactional `boolean_compound_regions` preserves pairwise-disjoint fuse members with total identity lineage, distributes intersect over member pairs, and distributes a single cut tool over every target member. Native and direct/batch WASM witnesses cover all three operations; overlap requiring a merge and multi-tool Cut refuse typed until recursive lineage composition exists. Together with PR #218 this closes the Issue 4.6 exit gate while retaining the old single-Solid APIs as compatibility surfaces.
- P4.7 wire bodies — IMPLEMENTED 2026-09-02 in PR #222: `body_length` and the existing copy/transform paths treat Wire as a first-class dimensional body; additive arena v5 preserves standalone root order, duplicates, exact rewrite, and v1–v4 compatibility through native/direct WASM codecs; and transactional `sweep_wire` turns a validated closed planar wire into a validation-gated solid without aliasing the input. Native/direct/batch witnesses pin exact perimeter and prism volume; open and non-planar profiles refuse typed without topology mutation. This closes the Issue 4.7 exit gate and completes the M4 implementation sequence.
- P5.1 variable-radius walking — IMPLEMENTED 2026-09-02 in PR #226: standard constant/linear/S-curve laws now prove exact whole-domain extrema, refuse tolerance collapse before topology work, and return typed `RadiusTooLarge` when their maximum reaches a caller-supplied exclusive local limit. The walker validates every station; its straight-edge perpendicular-plane linear band matches the analytic ruled quarter-circle surface and closed-form removed volume, and S-curve sections match the law plus both support tangencies. The builder no longer reduces opaque custom callbacks to endpoint-linear interpolation, but arbitrary between-sample certification and trimmed-solid assembly remain explicitly unqualified for later M5 work.
- P5.2 curved-support rims — IMPLEMENTED 2026-09-03 in PR #228: qualified analytic closed rims on cylinder/cone, cylinder/sphere, cone/cone, and segmented cylinder/cylinder supports assemble exact toroidal shoulders where provable and periodic walking-NURBS bands otherwise. Imported-resize, volume, topology, and direct/batch WASM witnesses are green; unsupported support combinations and closed legacy spines fail typed.
- P5.3 general vertex blends — IMPLEMENTED 2026-09-03 in PR #231: same-radius planar N-way corners with one connected material-side orientation produce analytic sphere caps, cylindrical stripes, and trimmed ellipse runouts. Box and pyramid witnesses pin G1, watertightness, volume, deterministic direct/batch WASM parity, and fail-closed mixed-side behavior; non-planar and variable-radius corners remain unqualified.
- P5.4 setbacks — IMPLEMENTED 2026-09-03 in PR #232: physical per-end distances crop straight-spine variable bands, whose laws are normalized over the active interval. The qualified planar 3+-way subset requires every incident stripe to meet one common radius with zero endpoint slope and one tangent ball; exact cubic-Hermite bands meet an exact sphere cap within angular tolerance. The three-edge box pins 1 mm result stations, mesh/B-Rep volume agreement, exact census output, direct/batch WASM parity, and transactional `setback-mismatch`; general mixed-radius junctions and curved-spine arc-length inversion remain unqualified.
- P5.5 overflow and cliff handling — MERGED 2026-09-03 in PR #235: the declared v2 policy is stop-at-cliff. Planar outer boundaries and inner-loop obstacles, closed-rim wall exhaustion, two bands consuming one wall, and inward cap collapse return transactional `CliffEncountered` with edge, face, requested radius, and locally available radius; direct and structured-batch WASM use stable `cliff-encountered`. Exact-width box and two-rim thin-wall witnesses pin both sides with closed/manifold B-Rep, watertight mesh, independent volume agreement, and unchanged-input refusals. PR #238 supplies a bounded plane/cylinder re-limitation substrate, but the blend caller does not use it and actual rollover remains unqualified.
- P5.6 face-face blend sheets and hold lines — MERGED 2026-09-03 in PR #236: one convex, hole-free, straight-edged planar face per set produces a validated standalone exact cylindrical Sheet when the carrier planes are transversal, edges are disjoint, and both contacts remain inside the bounded supports. A hold line must match one complete analytic contact. The perpendicular radius-1 witness pins exact `5 pi` area, independent mesh integration, scale/translation, direct/batch WASM parity, and transactional `unsupported-face-face-blend`; multi-face, curved/holed-support, partial-hold, and source-body trimming cells remain unqualified.
- P5.7 offset self-intersection removal — MERGED 2026-09-04 in PR #237: the shell path recognizes one fully collapsed disconnected inner component of a closed hole-free straight-edged uniform-width six-edge orthogonal L-prism only after matching the retained profile, cap inversion, prism topology, independent manifold closure, strict containment, and exact equality with the construction-proven generated-face set, then removes the empty region and its evolution outputs transactionally. A 4x4x2 L-bracket with one-unit arms retains its 0.4-wall cavity at exact volume 12.512 and excises the folded 0.6-wall cavity at exact/mesh volume 14 across 1e-3/1/1e3 scale and direct/both-batch WASM. Other profiles, connected/open, partial/multiple, holed, curved, and general intersecting cells remain typed or Unqualified.
- P6.1 replace-surface re-limitation — MERGED 2026-09-04 in PR #238: the core native operation replaces one coherently oriented plane or one same-axis coaxial inward-facing bore cylinder, re-intersects planar/cylindrical neighbors without changing topology, retains a total source/result face map, re-derives exact edge trims including closed Circle→Ellipse, and registers a fresh authoritative-domain p-curve on every result coedge. Tilted bored-cap and doubled-bore fixtures pin validation, two uses per edge, welded watertightness, B-Rep/independent-mesh volume, p-curve residuals, 1e-3/1e3 translated scale, and typed exact rollback for boundary crossings. General surface changes, bosses, NURBS supports, topology changes, blend rollover, WASM, and evolution remain later cells.
- B12 non-planar cap holes — IMPLEMENTED 2026-09-04: sweep and pipe preserve disjoint rectangular iso-parametric holes on four-sided bilinear caps. A translated saddle annulus matches an independently extruded planar annulus in converged volume while retaining two holed NURBS caps, a closed/manifold B-Rep, watertight tessellation, and an open center under point classification; direct WASM and `executeBatch` repeat the witness. Off-surface and broader trims fail before a cap is emitted, and loft now refuses profiles with holes instead of silently discarding their inner wires.
- B12 annular-Coons cell (PR #484): the same hole class is certified on n-sided Coons-carrier sweep/pipe caps as single trimmed faces (certification, tessellation, and volume are carrier-generic), winning over cap-then-subtract, which refuses exact for want of a NURBS-cap boolean arm. `crates/operations/tests/qualify_b12_annular_coons.rs` pins the hexagon section at 1e-3/1/1e3 (validate-clean, watertight mesh, volume within 1e-6 of the Cavalieri closed form, direct/batch WASM parity) plus the circular-annulus ground truth and a pinned typed refusal for true circular trims — a planar section of a genuine bilinear patch is never a circle, so no exact circular trim exists to certify.
...
- Remus K-S4 native-serialization fuzz slice — CLOSED 2026-08-31 in PR #173: `fuzz/fuzz_targets/arena_roundtrip.rs` round-trips bounded box/cylinder arena documents (duplicate roots, shared-shell aliases, compound member aliases, hostile tolerances, attributes) against per-position closed-form oracles and byte-identical re-serialization, with typed non-mutating refusal of corrupted references; pinned serde_json `float_roundtrip` as load-bearing for exact f64 replay. Curve-intersection and offset-specific fuzzing remain.
- B5 offset face provenance — CLOSED 2026-09-02 in PR #224: the default intersection-joint V2 offset retains and validates its exact one-to-one source-face map, journals it transactionally as construction evolution, and exposes matching direct/batch `offsetJournaled` WASM contracts. Plane displacement, 3x3x3 closed-form volume, persistent-reference chaining, atomic refusal, and WASM package oracles pin the claim; arc-joint and self-intersection-removal variants fail closed because their richer generated/replaced-face provenance remains future work.
- B6 primitive matrix — CLOSED 2026-09-03: box, cylinder, pointed cone, frustum, sphere, torus, and ellipsoid pass native/direct/batch scale and complete-postcondition oracles in `qualify_primitives.rs` and WASM `qualify_primitives_tests.rs`; the invalid matrix found and closed non-finite box/sphere/torus acceptance. The ellipsoid follow-up fixed four oracle-discovered defects: both transformed sphere faces selected the north patch, whole hemispheres were sampled instead of kept as exact rational patches, generic NURBS CDT accepted an empty result and independently re-sampled the shared rim, and NURBS bounds shrank the polar cap to its equator. The permanent `ellipsoid-tessellation-scale.json` replay pins volume, full bounds, and exact watertight mesh results across 1e-3/1/1e3.
- B15 cylinder-wall pocket orientation — CLOSED 2026-09-04 in PR #255: selected split cylinder loops could be geometrically closed yet use the carrier's wrong stored winding, so duplicate-edge merge paired every pocket rim with the same effective sense on its adjacent face. Non-fuse assembly now samples selected cylinder wires before merge, keeps outer loops aligned with the stored surface normal, and keeps inner loops opposed. The unignored `regress_parallel_boss_band_sections.rs` matrix covers box/cylinder tools on both wall sides under exact Cut and Intersect with dual validation, closed-form volumes, material-side classifications, and welded-mesh edge-use oracles.

## The deferred-items table, closure narratives, and digs (verbatim)
...
|---|---|---|
| **Tangent section circle on a quadric lateral vs. box walls — STALE-VERIFIED CLOSED (2026-08-20, this fork): the whole landscape is clean.** All four `diag_*tangency*` probes pass CLEAN (`diag_tangency_count` 0/1/2/4 walls, `diag_tangency_epsilon_band` −1e-3→+1e-3 including exact 0 and +1e-9, `diag_cone_box_tangency_sweep`, `diag_cylinder_box_tangency`) with analytic face counts, and `cone_union_box_should_be_analytic` is un-ignored and green — the 2-wall AND 4-wall cases both closed with the shipped tangency family, so the old "2-wall needs its own trigger" caution is history. The diag probes stay `#[ignore]` as print-only landscape diagnostics; re-run them (`-- --ignored --nocapture`) before ever re-opening this row. | algo/GFA | Census reconciled the same day — **AMENDED 2026-08-24, that reconciliation was sampled one operator per geometry pair.** `approx_census` tested each pair under a single operator, and in every case it happened to be one that passed, so "no boolean fallback rows" described the sampling rather than the engine. The expanded census exposed three boolean fallbacks hiding one operator away from rows already in it: `box / sphere ∪` (1192 planar faces), `cyl / cyl (perp cross) ∩` (70), and `torus / box ∩` (312). P-Class 2.3 has since closed the Steinmetz row; the other two remain honest mesh approximations. The non-boolean FALLBACKs are unchanged and still filter-excluded (NURBS-loft offset refit, fillet-v2 torus vertex blend, chamfer-v2 torus, offset_face SSI trim, pyramid corner patch). |
| **Steinmetz intersect — CLOSED 2026-09-02 by P-Class 2.3.** The historical `approx_census` row for equal-radius perpendicular cylinder ∩ cylinder returned 70 all-planar fallback faces and measured 0.80% off the closed form; across radii it ranged from 58 to 90 fallback faces. The seam is exactly two planar ellipses, so this was an integration gap rather than a representational limit. | algo/GFA + math | Phase FF now consumes the closed-form `exact_cylinder_cylinder` oracle and splits both ellipses at their two shared pinch points and half-span midpoints. The periodic splitter assembles six cylinder patches on eight authoritative ellipse arcs; validation, `16r³/3` volume, rigid motion, three mesh deflections, WASM exact-only execution, and the normalized census all pass. Fuse and cut remain exact with the explicit seam, moving to eight and seven faces respectively. |
| **Ellipse sections spanning > π — CLOSED as a guarded contract (2026-08-20, this fork; pin `phase_ff.rs::sampled_trim_never_emits_open_conic_arc_spanning_pi`)** | algo/GFA | The named durable fix shipped: `trim_closed_curve_to_inboth_arc`'s periodic branch (the SAMPLED closed-section trim — the exact-crossing emitter already split) now splits any ≥ π Circle/Ellipse window into < π sub-arcs. REACHABILITY MEASURED FIRST: an env-gated trace over the ENTIRE remus-io fixture corpus plus an oblique large-cutout sweep emitted ZERO > π arcs, so the split cannot perturb any calibrated chain (it never executes on them) — that measurement, not review, is the safety proof; re-measure the same way before touching this branch again. No failing repro ever existed; the close is the contract guard, not a defect fix. |
...
DISCOVERED and CLOSED same fork (2026-08-20, pin
`shell_op/tests.rs::shell_emits_no_same_sense_edge_pairs` covering the Planar,
CylindricalFace, and Surface arms; the fuse test's pair assertion strengthened to
zero): `shell_op` carried 64 same-sense rim pairs on the cavity lateral — TWO
independent orientation-emission defects. (1) Phase 4 gave the curved inner-face
specs BOTH a reversed vertex winding AND `reversed: !concave` — a double flip of the
effective traversal (`is_forward != is_reversed`); the reversed winding is the flip
mechanism ONLY for `FaceSpec::Planar` (assembled un-reversed with an explicit flipped
normal), while `CylindricalFace`/`Surface` flip via the face flag and must keep the
...
minus a small inscribed-mesh undercount); the reference kernel's 9500.0 matches neither the
...
## Pre-integration quadric and tangency checkpoint (archived 2026-09-09)

Moved from the active roadmap after #307/#308/#312/#338 merged. The following
text preserves the original measurements and caveats; its references to
unintegrated branches and pending checks are historical, not current status.
See [the current audit](evolution-audit.md) before selecting new work.
...
## Direct-edit, healing, and correctness slice reports (archived 2026-09-09)

Moved verbatim from `roadmap.md` §H on 2026-09-09. These are pre-integration
slice reports; everything below landed through #278, #285, #308, and #338.
Statements such as "not yet merged" or "the next slice" describe those
historical checkpoints. Current disposition lives in `evolution-audit.md`.
