...
| `unsupported` | The operation is well-formed but the capability cell is declared unsupported. Always cites the cell (family + configuration). | `unsupported_configuration`, existing typed refusals such as `RadiusTooLarge`, `cliff-encountered`, `unsupported-face-face-blend`, unsupported-support-pair |
