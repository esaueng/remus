...
| Boolean   | Fixed local fuse of half-overlap unit boxes, plus refused self-cut, each with 50/200/400 preexisting unit boxes; native direct and committed WASM batch | Fuse: 1.5 volume, 1.5-wide bounds, untouched solid intact. Refused: typed `operation_failed`/`internal` error with identical code on both runtimes; operands and untouched solid bit-identical afterward |
...
- Each boolean sample gets a freshly seeded kernel with `size` parked boxes plus two operands. Seeding is outside the timed total but its cost is reported. The timed total is snapshot (checkpoint) + kernel (fuse, including its internal transaction snapshot, or refused self-cut classification plus rollback) + validation (volume, bounds, rollback re-proof) + serialization (whole-document arena bytes) + teardown (kernel drop / `free`). Face-census and classification-probe oracles live in Rust integration tests, not on the binding surface, so the runner asserts volume, bounds, error codes and rollback state only.
...
The [workload manifest](../../scripts/performance/workloads.json) reuses the committed fixture and its existing regression reference; it downloads no new corpus. Existing [gauntlet manifests](../../tools/gauntlet/manifests) and [scorecard tooling](../../tools/gauntlet/README.md) remain the wider corpus inventory. The boolean family closes the success/refused document-scale gap; tessellation, queries/measurements, sketch, checkpoint/restore lifecycles, broader models, rendering and browser engines remain explicit gaps.
