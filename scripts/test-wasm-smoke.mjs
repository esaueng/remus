...
// and the refusal must not silently damage the imported body.
{
  ...
  // crates/io/tests/resize_blend_step.rs. Densifying curved NURBS trim bands
  // removes inward wall chords and moves this closer to the finer mesh and
  // independent integrator; retain the existing 0.01 repeatability tolerance.
  assert.ok(Math.abs(hammerKernel.volume(solid, 0.01) - 50_241.267_131) <= 0.01);
