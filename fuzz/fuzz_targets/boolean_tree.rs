...
#[cfg(not(test))]
fuzz_target!(|node: Node| run_case_with_nudge(node, check_root_nudge_stable));

fn run_case_with_nudge(node: Node, check_nudge: impl FnOnce(&Node)) {
    ...
    // Compare both outcomes even when the original root boolean refused.
    check_nudge(&node);
...
mod tests {
    ...
    #[test]
    fn refused_root_still_runs_nudge_comparison() {
        use super::shapegen::{self, BoolKind, Node, Prim};
        let leaf = Node::Leaf(Prim::Cuboid {
            dx: 0,
            dy: 0,
            dz: 0,
        });
        let node = Node::Combine(BoolKind::Cut, Box::new(leaf.clone()), Box::new(leaf));
        assert!(shapegen::eval_quiet(&mut Topology::new(), &node).is_err());
        super::check_root_nudge_stable(&node);
        let called = std::cell::Cell::new(false);
        super::run_case_with_nudge(node, |_| called.set(true));
        assert!(called.get(), "a refused root skipped nudge comparison");
    }

    #[test]
    fn successful_non_finite_measurements_are_findings() {
        assert!(inv::checked_volume::<()>(Err(())).is_none());
        assert_eq!(inv::checked_volume::<()>(Ok(32.0)), Some(32.0));
        for value in [f64::NAN, f64::INFINITY, f64::NEG_INFINITY] {
            assert!(std::panic::catch_unwind(|| inv::checked_volume::<()>(Ok(value))).is_err());
        }
    }

    #[test]
    fn closed_curves_require_authority_and_share_geometric_keys() {
        let mut topo = Topology::new();
        let circle =
            Circle3D::new(Point3::new(0.0, 0.0, 0.0), Vec3::new(0.0, 0.0, 1.0), 1.0).unwrap();
        let v = topo.add_vertex(Vertex::new(circle.evaluate(0.0), 1e-7));
        let bare = Edge::new(v, v, EdgeCurve::Circle(circle));
        let missing = topo.add_edge(bare.clone());
        assert_eq!(
            inv::edge_geom_key(&topo, missing),
            Err(inv::GeomKeyError::MissingAuthority)
        );
        let mut invalid = bare.clone();
        invalid.set_trim(Some((0.0, f64::NAN)));
        let invalid = topo.add_edge(invalid);
        assert!(inv::edge_geom_key(&topo, invalid).is_err());
        let mut closed = bare;
        closed.set_trim(Some((0.0, std::f64::consts::TAU)));
        let a = topo.add_edge(closed.clone());
        let b = topo.add_edge(closed);
        let key = inv::edge_geom_key(&topo, a).unwrap();
        assert_eq!(key, inv::edge_geom_key(&topo, b).unwrap());
        assert!(key.point_edge.is_none());
        assert_ne!(key.ends.0, key.mid);
    }

    #[test]
    fn nudge_refusal_success_flips_are_findings_in_both_directions() {
        let mut topo = Topology::new();
        let solid = remus_operations::primitives::make_box(&mut topo, 1.0, 1.0, 1.0).unwrap();
        let success = inv::NudgeOutcome::Success {
            census: inv::census(&topo, solid).unwrap(),
            volume: 1.0,
        };
        let refused = inv::NudgeOutcome::Refused {
            variant: "ExactOnlyUnattainable",
            reason: "injected refusal".into(),
        };
        for (a, b) in [(&refused, &success), (&success, &refused)] {
            assert!(
                std::panic::catch_unwind(|| inv::assert_nudge_stable("injected", a, b)).is_err()
            );
        }
        inv::assert_nudge_stable("refused", &refused, &refused);
        inv::assert_nudge_stable("success", &success, &success);
    }
}
