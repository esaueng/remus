...
//!
//! Builds a bounded unplaced primitive leaf from `shapegen` and tessellates it at two deflections. The oracle is
...
//!   B-Rep complexity is gated by the census before meshing; mesh-level
...
#[cfg(not(test))]
fuzz_target!(|prim: shapegen::Prim| run_case(prim));

fn run_case(prim: shapegen::Prim) {
    ...
    // Two deflections a factor of 4 apart, passed directly to the
    // tessellator without the volume measurement path's deflection clamp.
...
/// target finding. The census limits B-Rep complexity before
...
mod tests {
    ...
    #[test]
    fn committed_seeds_cover_all_five_primitives() {
        let seeds: [&[u8]; 5] = [
            include_bytes!("../corpus/tessellation/box-basic"),
            include_bytes!("../corpus/tessellation/cylinder-basic"),
            include_bytes!("../corpus/tessellation/cone-basic"),
            include_bytes!("../corpus/tessellation/sphere-basic"),
            include_bytes!("../corpus/tessellation/torus-basic"),
        ];
        for (index, data) in seeds.into_iter().enumerate() {
            let prim = shapegen::Prim::arbitrary(&mut arbitrary::Unstructured::new(data)).unwrap();
            let variant = match prim {
                shapegen::Prim::Cuboid { .. } => 0,
                shapegen::Prim::Cylinder { .. } => 1,
                shapegen::Prim::Cone { .. } => 2,
                shapegen::Prim::Sphere { .. } => 3,
                shapegen::Prim::Torus { .. } => 4,
            };
            assert_eq!(variant, index, "seed must represent its named primitive");
            run_case(prim);
        }
    }
}
