    ...
    for c in case.constraints {
        let r: Result<_, _> = match c {
            Constr::Coincident { a, b } => {
                sys.add_constraint(Constraint::Coincident(at(a), at(b))).map(|_| ())
            }
            Constr::Distance { a, b, d } => {
                sys.add_constraint(Constraint::Distance(at(a), at(b), mag(d))).map(|_| ())
            }
            ...
            Constr::FixX { a, v } => sys.add_constraint(Constraint::FixX(at(a), coord(v))).map(|_| ()),
            Constr::FixY { a, v } => sys.add_constraint(Constraint::FixY(at(a), coord(v))).map(|_| ()),
        };
