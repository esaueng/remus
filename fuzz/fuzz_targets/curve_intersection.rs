...
#[cfg(not(test))]
fuzz_target!(|data: &[u8]| run_case(data));
...
fn run_case(data: &[u8]) {
    ...
    for hit in &hits {
        assert!(
            hit.point.x().is_finite() && hit.point.y().is_finite() && hit.point.z().is_finite(),
...
mod tests {
    ...
    #[test]
    fn committed_seed_reaches_a_plane_hit() {
        let data = include_bytes!("../corpus/curve_intersection/plane-hit");
        let mut u = Unstructured::new(data);
        let curve = build_curve(&mut u).unwrap();
        let surface = build_surface(&mut u).unwrap();
        let hits = intersect_curve_surface(&curve, &surface, 1e-7).unwrap();
        assert!(!hits.is_empty(), "seed must exercise the hit oracle");
        assert!(
            hits.iter()
                .any(|hit| (hit.point - Point3::new(0.0, 0.0, 0.0)).length() < HIT_TOL)
        );
        run_case(data);
    }
}
